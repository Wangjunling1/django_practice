...
from PowerRing import render_question_dict_for_se
...
class QuestionRenderView(QuestionFilterUtils, View):
    '''用于从指定的数据表中获取指定question_id的题目
    '''
    @required_login
    def get(self, request, db_table, question_id):
        jr = JsonResult()
        question = self.get_question_from_db_table(
            db_table, question_id
        )
        if not question:
            jr.not_exist()
            return jr.to_http_resp()
        question_html = \
                render_question_dict_for_se(question)
        return HttpResponse(question_html, content_type="text/html")



class QuestionView(QuestionFilterUtils, View):
    '''用于从指定的数据表中获取指定question_id的题目
    '''
    @required_login
    def get(self, request, db_table, question_id):
...
class QuestionDetailView(QuestionFilterUtils, View):
    ...
    def get(self, request, search_condition):
        ...
        if not question:
            ...
            return jr.to_http_resp()
        question_html = render_question_dict_for_se(
            question
        )
        jr.success({
    ...
    def get(self, request):
        ...
        if form.is_valid():
            try:
                ...
                new_question_html = render_question_dict_for_se(
                    new_question
                )
                res_data = {
                    'question': {
    ...
    def get(self, request):
        ...
        if form.is_valid():
            ...
            for question in question_lst:
                question_html = render_question_dict_for_se(question)
                res_data.append({
                    'html': question_html,
                    'question_id': question.question_id
                })
...
class QuestionBgiqView(QuestionFilterUtils, View):
    ...
    def get(self, request):
        ...
        if form.is_valid():
            ...
            try:
                ...
                jr.invalid_params(str(err))
                return jr.to_http_resp()
            list_type= {question_lst[0].id:question_lst[0].html,
                        question_lst[1].id: question_lst[1].html}
            res_data = []
            for id,question in list_type.items():

                data_dict=self.bigq_transform_question(question,question_lst[0].source)

                question_html = render_question_dict_for_se(data_dict)
                res_data.append({
                    'html': question_html,
                    'question_id': id
                    })
    ...
    def _get_question_lst(self, form_data):
        ...
        # 1. 如果有question_url， 直接返回spider_url对应的题目
        if form_data.get('question_url'):
            return self.get_specified_questionBigq(
                db_table, form_data['question_url'], size=size
                )

        # 2. 随机筛选题目
        if form_data.get('filter_method') == 'random':
            return self.get_random_questionBigq(
                db_table,
                form_data['min_id'],
                form_data['max_id'],
                size=size
        ...
        # 4. 获取上一题或者下一题
            ...
            return self.get_sibling_questionBigq(
                db_table,
                form_data['min_id'],
                form_data['max_id'],
                form_data['current_question_id'],
            ...
            raise UnknownRequest('未知服务')
    # 将bigq格式类型转换为question格式类型
    def bigq_transform_question(self,data_dict,source):
        if len(data_dict)>2:
        ...
        if len(data_dict)>3:
            question_dict = {
                ...
                'question_html': f'''{data_dict.get('content','')}''',
                'option_html': f'''{True and data_dict.get('options','') or ''}''',
                'answer_all_html': f'''{data_dict.get('answers','')}''',
                'jieda':f'''{data_dict.get('solution','')}''',
                'fenxi': f'''{data_dict.get('analyse','')}''',
                'dianping': f'''{data_dict.get('comment','')}''',
                }
        else:
            question_dict = {
                'spider_source': source,
