...
from question_viewer.models import (
    ...
    get_model_for_specified_tableBigq,
    Question
)
...
class QuestionFilterUtils(object):
    ...
    #新加对于Bigq的操作
    @catch_common_error
    def get_sibling_questionBigq(self,
                             db_table,
                             min_id,
                             max_id,
                             current_question_id,
        ...
        model = get_model_for_specified_tableBigq(db_table)
        query_set = self._get_range_limited_query_setBigq(
            model, min_id, max_id
            )

        if which_sibling == 'previous':
            query_set = query_set.filter(
                id__lt=current_question_id
                ).order_by('-id')
        else:
            query_set = query_set.filter(
                id__gt=current_question_id
                ).order_by('id')
        return query_set[:size]
    ...
    @catch_common_error
    def get_question_from_db_table(self,
                                   db_table,
                                   question_id):
        '''根据question_id以及db_table获取指定的题目
        '''
        model = get_model_for_specified_table(db_table)
        return model.objects.filter(question_id=question_id).first()
    ...
    @catch_common_error
    def get_question_from_table(self,
                                db_table,
                                question_id):
        '''根据传入的db_table以及question_id获取对应的题目
        '''
        model = get_model_for_specified_table(db_table)
        return model.objects.filter(question_id=question_id)[:2]



    """下面是对于Bigq的接口"""

    def _get_range_limited_query_setBigq(self,
                                     model,
                                     min_id,
                                     max_id):
        '''获取限制了id范围的query_set
        ...
        query_set = model.objects
        if isinstance(min_id, int):
            query_set = query_set.filter(id__gte=min_id)
        if isinstance(max_id, int):
            query_set = query_set.filter(id__lte=max_id)
        return query_set

    # 新加的接口bigq
    @catch_common_error
    def get_questionBigq(self,
                         db_table,
                         min_id,
                         max_id,
                         ordering='asc',
        ...
        # 新加的接口bigq
    @catch_common_error
    def get_specified_questionBigq(self,
                                   db_table,
                                   spider_url,
                                   size=2):
        '''根据spider_url获取指定的题目
        '''
        model = get_model_for_specified_tableBigq(db_table)
        return model.objects.filter(key=spider_url)[:size]
    ...
    def _get_question_id_rangeBigq(self,
                               model,
                               min_id,
                               max_id):
        '''获取id的范围
        ...
        min_id_item = model.objects.order_by('id').first()

        if not min_id_item:
            raise EmptyModel('数据表为空数据表')
        real_min_id = min_id_item.id
        max_id_item = model.objects.order_by('-id').first()
        real_max_id = max_id_item.id
    ...
    # 新增加对于bigq随机
    @catch_common_error
    def get_random_questionBigq(self,
                                db_table,
                                min_id,
                                max_id,
                                size=2):
        ...
        model = get_model_for_specified_tableBigq(db_table)
        min_id, max_id = self._get_question_id_rangeBigq(
            model, min_id, max_id
            )
